...
export default async function SearchPage({ params }: { params: any }) {
    const results = await moviedb.searchMovie(params.query.replace(/-/g, ' '))

    return (
        <div className="mt-9">
            <h1 className="text-2xl font-bold mx-8 my-4">{results.results.length > 0 ? ('Resultater for') : 'Fant ingen resultater for'} <i>{params.query.replace(/-/g, ' ')}</i></h1>
