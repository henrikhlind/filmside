...
export default async function Home() {
  ...
  const westernMovies = await moviedb.listInfo(137403)
  ...
  return (
    <div className="-mt-20">
      <div className="relative">
        ...
        <div className="absolute h-[90vh] inset-0 bg-gradient-to-t from-slate-50 dark:from-stone-950 via-transparent"></div>
        <div className="absolute bottom-10 w-1/2 text-center left-1/2 -translate-x-1/2">
          ...
          <div className="flex flex-row gap-3 my-3 items-center justify-center">
            <Link href={'movies/' + showcaseMovie.id}>
              <button className="border rounded-full border-grey-darkest px-6 py-3 font-sans text-slate-50 font-medium flex items-center hover:bg-slate-50 hover:border-slate-50 hover:text-black transition">Vis mer <BsArrowRightShort size="1.3rem"/></button>
            </Link>
          </div>
        </div>
      </div>
      <h1 className="text-2xl font-bold mx-8 my-4">IMDb Top 250</h1>
      <div className="grid grid-rows-3 lg:grid-rows-2  grid-flow-col gap-4 mx-6 my-4">
        {topRated.items.slice(12, 18).map((movie: { id: Key; backdrop_path: string; title: string }) => (
